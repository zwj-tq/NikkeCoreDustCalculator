...
export const TABLET_BREAKPOINT = 1079;
...
export function getLayoutDensityTokens(layoutMode, viewportWidth = 0) {
  const desktopTokens = {
...
export function hasLayoutModeChanged(currentMode, width) {
  return currentMode !== getLayoutMode(width);
}

export function getStrategySelectionKey(row) {
  return row.id ?? row.name;
}

export function ensureStrategyIds(strategies) {
...
export function buildMobileSummaryCards(rows, activeKey) {
  return rows.map((row) => ({
...
export function buildMobileDetailCards(rows) {
  return rows.map((row) => ({
...
export function getSummaryRenderMode(layoutMode) {
  return layoutMode === "mobile" ? "cards" : "table";
}

export function getDetailViewRenderMode(layoutMode, mobileDetailView = "cards") {
  if (layoutMode !== "mobile") {
    return "table";
  }
...
export function getDetailViewToggleLabel(mobileDetailView = "cards") {
  return mobileDetailView === "table" ? "查看卡片明细" : "查看完整表格";
}

export function getInitialSectionOpenState(sectionId, layoutMode, defaultOpen = true) {
...
export function getResponsiveSectionOpenState(
  sectionId,
  layoutMode,
  layoutOverrides = {},
  defaultOpen = true,
...
export function buildCompactStatusItems(snapshot) {
  return [
    {
      key: "bestStrategy",
      label: "最佳策略",
...
export function buildToolbarActionGroups(actions, layoutMode = "desktop") {
  if (layoutMode !== "mobile") {
    return {
      primary: [...actions],
      secondary: [],
...
export function buildSummaryTableCells(row) {
  return [
    String(row.name ?? ""),
...
export function buildDetailTableCells(row) {
  return [
    String(row.day ?? ""),
    String(row.level ?? ""),
...
export function buildCollectionCardFields(kind, row) {
  if (kind === "strategies") {
    return [
...
export function buildMobileCollectionCardFields(kind, row) {
...
export function buildCollectionCardHeader(kind, row) {
  if (kind === "strategies") {
    return {
      title: row.name ?? "",
      subtitle: row.type ?? "",
...
export function getMainlineTimelinePresentation(layoutMode = "desktop") {
  if (layoutMode === "mobile") {
    return {
      titleTop: 6,
      titleFontSize: 16,
...
export function buildMainlineScatterData(entries, activeIndex, layoutMode = "desktop") {
